...
Garner & Alley 對照研究（理解與記憶顯著較佳；原始論文報告 p<.01，見 .fact-check.md #3）。
...
- 陳述該頁的**單一訊息**，是一句完整的話。
- 英文標題：約 **8–14 words、≤2 行、sentence case、句尾不加句號**（Alley 原典；「8–14」指英文 word 數）。
- 中文標題：約 **15–25 字、≤2 行、句末不加句號、避免標點堆疊**（CJK 資訊密度較高，勿照英文字數直接套）。
- 左對齊、標題下留白（給內文/證據呼吸）。
